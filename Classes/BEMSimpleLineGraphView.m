...
/// The gesture recognizer picking up the tap in the graph view
    ...
    if (self.enablePopUpReport == YES && self.alwaysDisplayPopUpLabels == NO) {
      if ([self.delegate
        ...
        NSString *maxValueString = [NSString
            stringWithFormat:self.formatStringForValues,
                             [self calculateMaximumPointValue].doubleValue];
        NSString *minValueString = [NSString
            stringWithFormat:self.formatStringForValues,
                             [self calculateMinimumPointValue].doubleValue];
  ...
  if (self.enableYAxisLabel) {
    ...
    if (self.autoScaleYAxis == YES) {
      NSString *maxValueString =
          [NSString stringWithFormat:self.formatStringForValues, self.maxValue];
      NSString *minValueString =
          [NSString stringWithFormat:self.formatStringForValues, self.minValue];
...
#pragma clang diagnostic pop
      ...
      CGFloat YAxisLabelXOffset =
          self.overlapYAxisWithGraph ? 0 : self.YAxisLabelXOffset;
      ...
      if (dotValue != BEMNullGraphValue) {
        ...
        if (self.animationGraphEntranceTime == 0) {
          if (self.displayDotsOnly == YES)
            ...
            if (self.alwaysDisplayDots == NO || shouldHideDot)
              circleDot.alpha = 0;
            else
              circleDot.alpha = 1.0;
          }
        } else {
          if (self.displayDotsWhileAnimating && !shouldHideDot) {
...
- (void)drawLine {
  ...
  line.referenceLineWidth = self.widthReferenceLines ? self.widthReferenceLines
                                                     : (self.widthLine / 2);
  ...
  line.lineDashPatternForReferenceYAxisLines =
      self.lineDashPatternForReferenceYAxisLines;
  line.lineDashPatternForReferenceXAxisLines =
  ...
  if (self.enableReferenceXAxisLines || self.enableReferenceYAxisLines) {
    ...
    line.verticalReferenceHorizontalFringeNegation =
...
- (void)drawXAxis {
  ...
  self.backgroundXAxis =
      [[UIView alloc] initWithFrame:[self drawableXAxisArea]];
...
#pragma clang diagnostic pop
    ...
    if (numberOfGaps >= (numberOfPoints - 1)) {
      ...
      UILabel *lastLabel =
          [self xAxisLabelWithText:lastXLabel atIndex:numberOfPoints - 1];
      lastLabel.frame =
...
- (UILabel *)xAxisLabelWithText:(NSString *)text atIndex:(NSInteger)index {
  ...
  if (self.positionYAxisRight) {
    positionOnXAxis =
        (((self.frame.size.width - YAxisLabelXOffset) / (numberOfPoints - 1)) *
         index) +
        horizontalTranslation;
  } else {
    positionOnXAxis =
...
- (void)drawYAxis {
  ...
  if (self.positionYAxisRight) {
    ...
    xValueForCenterLabelYAxis =
        (self.YAxisLabelXOffset / 2) + self.marginLeftYAxisLabel;
    textAlignmentForLabelYAxis = NSTextAlignmentRight;
  }
...
- (NSString *)kFormatForNumber:(NSNumber *)number {
  if (number.doubleValue < 1000)
    return [NSString
        stringWithFormat:self.formatStringForValues, number.doubleValue];

  CGFloat result = number.doubleValue / 1000;

  NSString *kFormatStringValues =
      [NSString stringWithFormat:@"%@k", self.formatStringForValues];
...
- (CGRect)drawableGraphArea {
  ...
  CGFloat xOrigin = self.positionYAxisRight ? 0 : YAxisLabelXOffset;
...
- (CGRect)drawableXAxisArea {
  ...
  CGFloat xAxisXOrigin = self.positionYAxisRight ? 0 : YAxisLabelXOffset;
...
- (void)displayPermanentLabelForPoint:(BEMCircle *)circleDot {
  ...
  NSString *formattedValue =
      [NSString stringWithFormat:self.formatStringForValues, value.doubleValue];
...
- (UIImage *)graphSnapshotImageRenderedWhileInBackground:
...
- (void)selectPointIndex:(NSUInteger)index {
  UILabel *xAxisLabel =
      (UILabel *)[self.graphLabelsForXAxis objectAtIndex:index];

  self.touchInputLine.frame =
      CGRectMake(CGRectGetMidX(xAxisLabel.frame), 0, self.widthTouchInputLine,
                 self.frame.size.height - (self.XAxisLabelYOffset + 2));
  self.touchInputLine.alpha = self.alphaTouchInputLine;

  closestDot = [self closestDotFromtouchInputLine:self.touchInputLine];

  if ([self.delegate respondsToSelector:@selector(lineGraph:
...
- (void)handleGestureAction:(UIGestureRecognizer *)recognizer {
  ...
  if ([self.delegate respondsToSelector:@selector(lineGraph:hideDotAtIndex:)]) {
    self.touchInputLine.frame =
        CGRectMake(CGRectGetMidX(closestDot.frame), 0, self.widthTouchInputLine,
                   self.frame.size.height - (self.XAxisLabelYOffset + 2));
  ...
  if (self.enablePopUpReport == YES && closestDot.tag >= DotFirstTag100 &&
      closestDot.tag < DotLastTag1000 &&
      [closestDot isKindOfClass:[BEMCircle class]] &&
      self.alwaysDisplayPopUpLabels == NO) {
    [self setUpPopUpLabelAbovePoint:closestDot];
  }

  if (closestDot.tag >= DotFirstTag100 && closestDot.tag < DotLastTag1000 &&
    ...
    if ([self.delegate
      ...
      [self.delegate lineGraph:self
          didTouchGraphWithClosestIndex:((NSInteger)closestDot.tag -
                                         DotFirstTag100)];
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
      [self.delegate
          didTouchGraphWithClosestIndex:((int)closestDot.tag - DotFirstTag100)];
#pragma clang diagnostic pop
    }
  }
  ...
  if (!self.overlapYAxisWithGraph) {
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded) {
      if ([self.delegate respondsToSelector:
        ...
        [self.delegate lineGraph:self
            didReleaseTouchFromGraphWithClosestIndex:(closestDot.tag -
                                                      DotFirstTag100)];

      } else if ([self.delegate
                     respondsToSelector:
                         @selector(didReleaseGraphWithClosestIndex:)]) {
        [self printDeprecationWarningForOldMethod:
                  @"didReleaseGraphWithClosestIndex:"
                             andReplacementMethod:@"lineGraph:"
                                                  @"didReleaseTouchFromGraphWi"
                                                  @"thClosestIndex:"];

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        [self.delegate
            didReleaseGraphWithClosestIndex:(closestDot.tag - DotFirstTag100)];
#pragma clang diagnostic pop
      }
...
- (void)setUpPopUpLabelAbovePoint:(BEMCircle *)closestPoint {
  ...
  if ([self.delegate
          respondsToSelector:@selector(lineGraph:modifyPopupView:forIndex:)]) {
    [self.delegate lineGraph:self
             modifyPopupView:self.popUpView
                    forIndex:index];
  }
  self.xCenterLabel = closestDot.center.x;
  self.yCenterLabel =
  ...
  if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
    self.popUpLabel.text = [NSString
        stringWithFormat:@"%li%@",
                         (long)[dataPoints[(NSInteger)closestDot.tag -
                                           DotFirstTag100] integerValue],
  ...
  if (self.enableYAxisLabel == YES &&
    ...
    popUpViewCenter = CGPointMake(self.xCenterLabel - self.YAxisLabelXOffset,
                                  self.yCenterLabel);
  ...
  if (self.popUpView.frame.origin.y <= 2) {
    self.yCenterLabel =
        closestDot.center.y + closestDot.frame.size.height / 2 + 15;
    popUpViewCenter = CGPointMake(self.xCenterLabel,
                                  closestDot.center.y +
                                      closestDot.frame.size.height / 2 + 15);
  }
  ...
  if (!self.usingCustomPopupView) {
    ...
    NSString *formattedValue = [NSString
        stringWithFormat:self.formatStringForValues, value.doubleValue];
...
- (BEMCircle *)closestDotFromtouchInputLine:(UIView *)touchInputLine {
  ...
  for (BEMCircle *point in self.subviews) {
    if (point.tag >= DotFirstTag100 && point.tag < DotLastTag1000 &&
