...
- (void)viewDidLoad {
  ...
  /* This is commented out because the graph is created in the interface with
   this sample app. However, the code remains as an example for creating the
   graph using code.
   BEMSimpleLineGraphView *myGraph = [[BEMSimpleLineGraphView alloc]
   initWithFrame:CGRectMake(0, 60, 320, 250)];
  ...
  self.labelValues.text = [NSString
      stringWithFormat:@"%i", [[self.myGraph calculatePointValueSum] intValue]];
...
- (void)hydrateDatasets {
  // Reset the arrays of values (Y-Axis points) and dates (X-Axis points /
  // labels)
  ...
  for (int i = 0; i < 9; i++) {
    [self.arrayOfValues
        addObject:@([self getRandomFloat])]; // Random values for the graph
    if (i == 0) {
      [self.arrayOfDates
          addObject:baseDate]; // Dates for the X-Axis of the graph
    } else if (showNullValue && i == 4) {
      [self.arrayOfDates
          addObject:[self dateForGraphAfterDate:self.arrayOfDates[i -
                                                                  1]]]; // Dates
      // for the
      // X-Axis
      // of the
    ...
    totalNumber =
        totalNumber + [[self.arrayOfValues objectAtIndex:i]
                          intValue]; // All of the values added together
  }
}
...
- (IBAction)refresh:(id)sender {
  ...
  //    [self hydrateDatasets];
  //
  //    UIColor *color = [UIColor colorWithRed:31.0/255.0 green:187.0/255.0
  //    blue:166.0/255.0 alpha:1.0]; // set the default color
  //    if (self.graphColorChoice.selectedSegmentIndex == 0) color = [UIColor
  //    colorWithRed:31.0/255.0 green:187.0/255.0 blue:166.0/255.0 alpha:1.0];
  //    else if (self.graphColorChoice.selectedSegmentIndex == 1) color =
  //    [UIColor colorWithRed:255.0/255.0 green:187.0/255.0 blue:31.0/255.0
  //    alpha:1.0];
  //    else if (self.graphColorChoice.selectedSegmentIndex == 2) color =
  //    [UIColor colorWithRed:0.0 green:140.0/255.0 blue:255.0/255.0 alpha:1.0];
  //
  //    self.myGraph.enableBezierCurve = (BOOL)
  //    self.curveChoice.selectedSegmentIndex;
...
- (IBAction)addOrRemovePointFromGraph:(id)sender {
  if (self.graphObjectIncrement.value > previousStepperValue) {
    ...
    NSDate *newDate =
        [self dateForGraphAfterDate:(NSDate *)[self.arrayOfDates lastObject]];
...
- (IBAction)exitToRoot:(UIStoryboardSegue *)sender {
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  ...
  if ([segue.identifier isEqualToString:@"showStats"]) {
    ...
    controller.standardDeviation = [NSString
        stringWithFormat:@"%.2f",
                         [[self.myGraph calculateLineGraphStandardDeviation]
                             floatValue]];
    controller.average = [NSString
        stringWithFormat:@"%.2f", [[self.myGraph calculatePointValueAverage]
                                      floatValue]];
    controller.median = [NSString
        stringWithFormat:@"%.2f",
                         [[self.myGraph calculatePointValueMedian] floatValue]];
    controller.mode = [NSString
        stringWithFormat:@"%.2f",
                         [[self.myGraph calculatePointValueMode] floatValue]];
    controller.minimum = [NSString
        stringWithFormat:@"%.2f", [[self.myGraph calculateMinimumPointValue]
                                      floatValue]];
    controller.maximum = [NSString
        stringWithFormat:@"%.2f", [[self.myGraph calculateMaximumPointValue]
                                      floatValue]];
...
//- (NSInteger)numberOfGapsBetweenLabelsOnLineGraph:(BEMSimpleLineGraphView
//*)graph {
//    return 2;
//}
...
- (void)lineGraph:(BEMSimpleLineGraphView *)graph
    didTouchGraphWithClosestIndex:(NSInteger)index {
  self.labelValues.text = [NSString
      stringWithFormat:@"%@", [self.arrayOfValues objectAtIndex:index]];
  self.labelDates.text =
      [NSString stringWithFormat:@"in %@", [self labelForDateAtIndex:index]];
}

- (void)lineGraph:(BEMSimpleLineGraphView *)graph
    didReleaseTouchFromGraphWithClosestIndex:(CGFloat)index {
  //    [UIView animateWithDuration:0.2 delay:0
  //    options:UIViewAnimationOptionCurveEaseOut animations:^{
  //        self.labelValues.alpha = 0.0;
  //        self.labelDates.alpha = 0.0;
  //    } completion:^(BOOL finished) {
  //        self.labelValues.text = [NSString stringWithFormat:@"%i",
  //        [[self.myGraph calculatePointValueSum] intValue]];
  //        self.labelDates.text = [NSString stringWithFormat:@"between %@ and
  //        %@", [self labelForDateAtIndex:0], [self
  //        labelForDateAtIndex:self.arrayOfDates.count - 1]];
  //
...
- (void)lineGraphDidFinishLoading:(BEMSimpleLineGraphView *)graph {
  self.labelValues.text = [NSString
      stringWithFormat:@"%i", [[self.myGraph calculatePointValueSum] intValue]];
  self.labelDates.text = [NSString
      stringWithFormat:@"between %@ and %@", [self labelForDateAtIndex:0],
                       [self labelForDateAtIndex:self.arrayOfDates.count - 1]];
}
...
/*
 This section holds a bunch of graph customizations that can be made.  They are
 commented out because they aren't required.  If you choose to uncomment some,
 they will override some of the other delegate and datasource methods above.
...
//- (NSArray *)incrementPositionsForXAxisOnLineGraph:(BEMSimpleLineGraphView
//*)graph {
...
//    double roundedVal = round((min - offset) / roundPrecision) *
//    roundPrecision;
//    return roundedVal;
//}
//
...
//- (BOOL)lineGraph:(BEMSimpleLineGraphView *)graph
//    hideDotAtIndex:(CGFloat)index {
//  return index == 0 || index == self.arrayOfDates.count - 1;
//}
//
//- (BOOL)lineGraph:(BEMSimpleLineGraphView *)graph
//    hideLabelAtIndex:(CGFloat)index {
//  return index == 0 || index == self.arrayOfDates.count - 1;
//}
